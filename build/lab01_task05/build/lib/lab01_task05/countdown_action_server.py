...
class CountdownActionServer(Node):
    ...
    def goal_callback(self, goal_request):
        ...
        start_from = goal_request.start_from
        
        if start_from < 0:
            self.get_logger().warn(f'Rejecting goal - invalid start_from: {start_from}')
            return GoalResponse.REJECT
            
        self.get_logger().info(f'Accepting goal request: countdown from {start_from}')
