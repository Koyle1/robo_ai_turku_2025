...
# DIREKT NACH deiner Dijkstra-While-Schleife (Zeile 85) füge das ein:

def run_dijkstra():
    visited = set()
    q = PriorityQueue()
    q.put((0, start_waypoint_index, [start_waypoint_index]))

    while not q.empty():
        current_cost, current_waypoint, current_path = q.get()
        
        if current_waypoint in visited:
            continue
...
if __name__ == "__main__":
    ...
    if ros_path:
        ...
        for i, node in enumerate(waypoint_nodes):
            ...
            elif i == waypoint_indices[-1]:  # Letzter besuchter Waypoint
